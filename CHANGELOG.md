...
- feat: 版式合同升级为通用字段地图 v2，DOCX 精确记录表/行/列/段落坐标，XLSX 精确记录工作表/单元格坐标；同字段多位置完整保留
- feat: 模板规则保存时把 AI 语义策略与确定性坐标合并持久化，旧版合同自动迁移；生成前阻止未完成坐标定位的模板参与正式出件
- ui: 模板中心和版式合同编辑器展示精准定位数量、具体坐标及待复核状态
- feat: 模板导入改为先选文件再自动识别文种，复合站点资料包可识别多个子表单并作为一个文件一次生成
- fix: 模板结构识别不再把固定检查标准后的空段落误判为字段，候选数量由 20 项上限提升到 120 项
- feat: 同一项目字段支持映射到复合资料包的多个位置；长文本使用独立段落写入并随表格行高自动扩展
- fix: 项目旧样例值在表格同格、相邻值格和跨文本片段场景均可确定性替换；AI 仅补字段语义和规则，不覆盖本地结构坐标
- feat: 模板中心新增按工程专业管理的“站点资料包”，支持添加、预览、字段识别、规则配置和项目内显式选用 DOCX/XLSX 表单
- fix: 站点资料包使用独立物理目录与资源登记，不覆盖同文种普通专业模板，也不会在未选择时自动参与模板路由
- fix: AI 文档助手按“内置通用/当前项目专业/私人/用户自定义/站点资料包”完整展示资源，当前专业库即使为空也不再隐藏
- fix: 未手动选模板时优先自动路由当前项目专业模板；用户通用副本不再遮蔽内置模板资源
- fix: 所有模板来源统一注入项目专业强制约束，并补齐通信、电力 SOP 与正式保存跨专业术语门禁
- fix: 私人/企业实体模板生成时只保留已登记字段，禁止为凑字数追加模板外日期、人员、现场检查结果和后续计划
- fix: 用户模板的日期、天气设为事实提取时不再自动填入当天和外部天气；无来源字段在 AI 返回后强制留空
- fix: 复合资料包新增字段级事实守门，剔除无来源的巡视、旁站、安全结论、材料细节、“未发现问题”和签字内容
- fix: 模板结构分析过滤固定检查标准的截断句，事实字段 AI 优化不得升级为场景扩写
- fix: 明细表改为逐列占位符映射；已填写的旧项目样例值可原位替换为项目资料字段
- fix: 保留模板版式时仍统一兼容中文字体，减少 macOS/LibreOffice 中文缺字方框
- feat: 客户端在线更新源切换为 `micfree/project-management` Gitee 专用发行仓库，按平台和 CPU 架构选择 Release 附件并校验可信下载地址
- build: 新增 Gitee Release 创建/附件上传脚本，Windows CI 在标签发布时仅将安装包上传到 Gitee
- feat: AI 文档预览新增可点击“待补充/人工字段”状态、集中补充抽屉、文档内定位、项目资料同步和自动取数重试
- fix: 全部文种文件名改用固定文种/报告期摘要，主进程忽略用户输入、事由、customSummary 和前端自定义文件名
- fix: Windows 默认项目根目录从桌面迁移到“文档/项目文档管理系统/项目”，首页只展示已登记项目，不再加载桌面普通文件
- ui: 隐藏 Electron 原生 File/Edit/View/Window 菜单栏；展开主菜单后点击外部区域自动收起
- feat: 新建与编辑项目增加“实施区域”，自动注入 AI 项目事实上下文
- fix: 监理日志自动填写当天日期和星期；天气、气温缺失时记为“未记录”，不再阻断整份日志生成
- fix: 普通模板无占位符时明确进入结构分析；刷新私人模板库会自动登记资源管理器直接复制的 DOCX/XLSX
- fix: 内置文种模板及其私人副本自动继承随应用交付的 AI 规则，无需用户重新填写扩写词
- feat: 18 个内置通用模板按文种和字段补齐事实来源、专业写作维度、组织顺序、缺失策略与反编造边界
- test: 增加私人模板目录自动登记、内置字段规则完整性与专业深度回归测试
...
### 全局规则中心与规则分层
- feat: 模板中心新增独立“全局规则”入口，将所有文档共同约束从具体模板编辑器移出
- feat: 建立“系统安全底线 → 全局文档规则 → 业务专业规则 → 文种规则 → 字段规则”五层执行架构
- sec: 事实来源、防提示注入和审批隔离设为系统锁定底线，用户及模板规则不可关闭或覆盖
- refactor: AI 运行时统一从共享配置读取全局规则，正常、自定义和兜底生成路径使用同一规则来源
- ui: 系统执行详情改为生成流程、规则分层和字段数量摘要，原始规则下沉到高级折叠区
- test: 新增全局规则作用域、系统锁定、模板中心入口和运行时规则来源回归测试
...
- fix: 模板字段扫描与 DOCX 可视预览改为同时加载，系统/专业/私人模板点击后立即显示完整模板内容
- fix: AI 建议字段、用户定位新增字段与模板原生占位符统一渲染为模板原位可点击标签，点击后进入右侧 AI 扩写配置
- fix: 模板编辑三栏改为比例伸缩，预览高度随窗口自适应，在中等宽度窗口提前重排，避免后部面板被截断
- feat: 模板原文内点击目标段落即可原位添加占位符，移除底部固定添加栏；占位符误删支持会话内撤销恢复
- feat: 专业/私人/其他模板支持右键操作并将物理文件移到系统废纸篓
- fix: AI 文档助手的当前项目模板树补齐右键“移到系统废纸篓”，删除后立即刷新资源树
- feat: 模板映射支持按表格坐标点击空白单元格添加占位符并写回 DOCX；复杂表头和格式调整提供 Word/WPS 编辑后重新载入流程
- refactor: 内置模板改为严格“开箱即用”清单，仅保留同时具备模板文件、可识别占位符和专属 AI 扩写规则的 18 个文种；缺失或零占位符文种改由用户自定义
- chore: 移除已废弃的内置专业模板源文件及监理规划目录中 4 份零占位符底稿，防止新安装再次迁移未完善模板
- fix: 当前项目/专业/私人模板与系统模板同名时不再错误隐藏通用内置项，资源树始终完整展示 18 个开箱即用模板
- fix: 模板预览使用原始模板文件名打开临时副本，不再显示企业库内部 ID 前缀
- feat: AI 扩写界面右侧模板库节点默认收起；每份用户模板独立记录规则完成状态，未配置时发送前阻止生成并引导配置，保存后携带原输入返回项目继续扩写
- refactor: 全部模板统一到“文种名称模板.ext”命名，同一分类同一文种只保留一份正式模板，历史重复项进入清理归档，不再显示“变体”
- feat: 安装后模板统一落到“文档/项目文档管理系统/模板库”，按内置、专业、私人、其他分类；模板中心可直接打开该目录
- feat: 模板中心支持新增专业目录、整专业删除；删除时专业目录与全部模板一起移到系统废纸篓
- refactor: 左侧导航默认中文展开，主界面、模板中心、新建项目优先展示，诊断与多项目驾驶舱下沉；支持一键收起为图标
