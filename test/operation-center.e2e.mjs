...
async function main() {
  ...
  const audit = await call('template:audit', path.resolve('templates/通用/05_整改通知书/整改通知书模板.docx'))
  assert.equal(audit.success, true)
  assert.ok(audit.fields.length > 0)
  const visual = await call('doc:visualAudit', path.resolve('templates/通用/05_整改通知书/整改通知书模板.docx'))
